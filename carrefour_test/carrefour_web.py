...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
class Tomcat(object):
    def __init__(self):  # 去除了 tomcat_exe参数(多实例启动参数如:(self,tomcat_exe))
        self.tomcat_exe = "tomcat-" + "backend"  # mobile 可以换成 Tomcat_exe 参数
        ...
        self.dest_deploy_dir = "/install/deploy-backend/%s" % self.source_files.split('/')[2].split('.war')[0]
    ...
    try:
        if args.handle == 'log':
            ...
            while True:
                ...
                mobile_return_code = Handle.get_status_code(host=ipaddress_port, path='/login')
                if mobile_return_code == 200:
                    print "\033[32m %s Process Is Exist Service Is available Return Code:\033[0m" % Handle.tomcat_exe + "\033[31m%s\033[0m" % mobile_return_code + "\033[32mCheck URL:http://%s/login\033[0m" % ipaddress_port
                    break
            ...
            while True:
                ...
                mobile_return_code = Handle.get_status_code(host=ipaddress_port, path='/login')
                if mobile_return_code == 200:
                    print "\033[32m %s Process Is Exist Service Is available Return Code:\033[0m" % Handle.tomcat_exe + "\033[31m%s\033[0m" % mobile_return_code + "\033[32mRollback Last Deployment SuccessFul\033[0m"
                    break
        else:
